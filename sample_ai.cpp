...
//	while(1)
	int n = 0;
	cerra(2) << "Size: mov0:" << mov0.size() << endl;
	while(get_time() < 2)
	{
		cerra(2) << "Test " << n++ << "{\n";
		for (int i = 0; i < mov0.size(); i++)
		{
			...
			cerra(2) << "Size: mov1:" << mov1.size() << endl;
			for (int j = 0; j < mov1.size(); j++)
			{
				cerra(2) << j << " Searching mov1:{\n";
				mov1[j].print(2);
				memcpy(fmap, map, sizeof(map));
				Chessboard B0(CB[id].fork(fmap));
				Chessboard B1(CB[!id].fork(fmap));
				if(!(B0.MoveBoard(mov0[i]) && B1.MoveBoard(mov1[j]))) continue;
				for (int r = 0; r < 3; r++)
				{
					cerra(2) << "round " << r << ":{\n";
					vector <Cmove> mv0 = B0.GenerateMoves();
					if(!mv0.size()) break;
					int c0 = rand() % mv0.size();
					B0.MoveBoard(mv0[c0]);
					vector <Cmove> mv1 = B1.GenerateMoves();
					if(!mv1.size()) break;
					int c1 = rand() % mv1.size();
					B1.MoveBoard(mv1[c1]);
					cerra(2) << "B0 move:";
					mv0[c0].print(2);
					cerra(2) << "B1 move:";
